...
unsigned int (*nf_nat_cisco_sip_hook)(struct sk_buff *skb, unsigned int dataoff,
				const char **dptr,
				unsigned int *datalen) __read_mostly;
EXPORT_SYMBOL_GPL(nf_nat_cisco_sip_hook);

void (*nf_nat_cisco_sip_seq_adjust_hook)(struct sk_buff *skb, s16 off) __read_mostly;
EXPORT_SYMBOL_GPL(nf_nat_cisco_sip_seq_adjust_hook);

unsigned int (*nf_nat_cisco_sip_expect_hook)(struct sk_buff *skb,
...
EXPORT_SYMBOL_GPL(nf_nat_cisco_sip_expect_hook);

unsigned int (*nf_nat_cisco_sdp_addr_hook)(struct sk_buff *skb, unsigned int dataoff,
				     const char **dptr,
				     unsigned int *datalen,
				     unsigned int sdpoff,
				     ...
				     __read_mostly;
EXPORT_SYMBOL_GPL(nf_nat_cisco_sdp_addr_hook);

unsigned int (*nf_nat_cisco_sdp_port_hook)(struct sk_buff *skb, unsigned int dataoff,
				     const char **dptr,
				     unsigned int *datalen,
				     unsigned int matchoff,
				     unsigned int matchlen,
				     u_int16_t port) __read_mostly;
EXPORT_SYMBOL_GPL(nf_nat_cisco_sdp_port_hook);

unsigned int (*nf_nat_cisco_sdp_session_hook)(struct sk_buff *skb,
					unsigned int dataoff,
					const char **dptr,
					unsigned int *datalen,
					unsigned int sdpoff,
					const union nf_inet_addr *addr)
					__read_mostly;
EXPORT_SYMBOL_GPL(nf_nat_cisco_sdp_session_hook);

unsigned int (*nf_nat_cisco_sdp_media_hook)(struct sk_buff *skb, unsigned int dataoff,
				      const char **dptr,
				      unsigned int *datalen,
				      ...
				      __read_mostly;
EXPORT_SYMBOL_GPL(nf_nat_cisco_sdp_media_hook);
 ...
 */
int ct_cisco_sip_parse_request(const struct nf_conn *ct,
...
EXPORT_SYMBOL_GPL(ct_cisco_sip_parse_request);
...
int ct_cisco_sip_get_header(const struct nf_conn *ct, const char *dptr,
...
EXPORT_SYMBOL_GPL(ct_cisco_sip_get_header);
			       ...
			       unsigned int *matchoff, unsigned int *matchlen)
{
	...
	while (1) {
		ret = ct_cisco_sip_get_header(ct, dptr, dataoff, datalen,
					type, matchoff, matchlen);
		if (ret > 0)
			break;
		if (ret == 0)
 ...
 * pointer.
 */
int ct_cisco_sip_parse_header_uri(const struct nf_conn *ct, const char *dptr,
			    unsigned int *dataoff, unsigned int datalen,
			    enum sip_header_types type, int *in_header,
			    unsigned int *matchoff, unsigned int *matchlen,
			    union nf_inet_addr *addr, __be16 *port)
...
EXPORT_SYMBOL_GPL(ct_cisco_sip_parse_header_uri);
...
int ct_cisco_sip_parse_address_param(const struct nf_conn *ct, const char *dptr,
			       unsigned int dataoff, unsigned int datalen,
			       const char *name,
			       unsigned int *matchoff, unsigned int *matchlen,
			       union nf_inet_addr *addr)
...
EXPORT_SYMBOL_GPL(ct_cisco_sip_parse_address_param);

/* Parse numerical header parameter and return value, offset and length */
int ct_cisco_sip_parse_numerical_param(const struct nf_conn *ct, const char *dptr,
				 unsigned int dataoff, unsigned int datalen,
				 const char *name,
				 unsigned int *matchoff, unsigned int *matchlen,
				 unsigned int *val)
...
EXPORT_SYMBOL_GPL(ct_cisco_sip_parse_numerical_param);
 ...
 */
int ct_cisco_sip_get_sdp_header(const struct nf_conn *ct, const char *dptr,
			  unsigned int dataoff, unsigned int datalen,
			  enum sdp_header_types type,
			  enum sdp_header_types term,
			  unsigned int *matchoff, unsigned int *matchlen)
...
EXPORT_SYMBOL_GPL(ct_cisco_sip_get_sdp_header);
				 ...
				 union nf_inet_addr *addr)
{
	...
	ret = ct_cisco_sip_get_sdp_header(ct, dptr, dataoff, datalen, type, term,
				    matchoff, matchlen);
	if (ret <= 0)
		return ret;
				 ...
				 unsigned int mediaoff, unsigned int medialen)
{
	...
	typeof(nf_nat_cisco_sdp_port_hook) nf_nat_sdp_port;
	typeof(nf_nat_cisco_sdp_media_hook) nf_nat_sdp_media;
	...
	if (direct_rtp) {
		nf_nat_sdp_port = rcu_dereference(nf_nat_cisco_sdp_port_hook);
		if (nf_nat_sdp_port &&
	...
	nf_nat_sdp_media = rcu_dereference(nf_nat_cisco_sdp_media_hook);
		       ...
		       unsigned int cseq)
{
	...
	const struct sdp_media_type *t;
	int ret = NF_ACCEPT;
	typeof(nf_nat_cisco_sdp_addr_hook) nf_nat_sdp_addr;
	typeof(nf_nat_cisco_sdp_session_hook) nf_nat_sdp_session;

	nf_nat_sdp_addr = rcu_dereference(nf_nat_cisco_sdp_addr_hook);
	c_hdr = nf_ct_l3num(ct) == AF_INET ? SDP_HDR_CONNECTION_IP4 :
					     SDP_HDR_CONNECTION_IP6;

	/* Find beginning of session description */
	if (ct_cisco_sip_get_sdp_header(ct, *dptr, 0, *datalen,
	...
	for (i = 0; i < ARRAY_SIZE(sdp_media_types); ) {
		if (ct_cisco_sip_get_sdp_header(ct, *dptr, mediaoff, *datalen,
	...
	nf_nat_sdp_session = rcu_dereference(nf_nat_cisco_sdp_session_hook);
				    ...
				    unsigned int cseq)
{
	...
	int ret;
	typeof(nf_nat_cisco_sip_expect_hook) nf_nat_sip_expect;
	 ...
	 */
	if (ct_cisco_sip_get_header(ct, *dptr, 0, *datalen, SIP_HDR_EXPIRES,
			      &matchoff, &matchlen) > 0)
		expires = simple_strtoul(*dptr + matchoff, NULL, 10);

	ret = ct_cisco_sip_parse_header_uri(ct, *dptr, NULL, *datalen,
	...
	if (ct_cisco_sip_parse_numerical_param(ct, *dptr,
	...
	nf_nat_sip_expect = rcu_dereference(nf_nat_cisco_sip_expect_hook);
				     ...
				     unsigned int cseq, unsigned int code)
{
	...
	while (1) {
		...
		ret = ct_cisco_sip_parse_header_uri(ct, *dptr, &coff, *datalen,
		...
		ret = ct_cisco_sip_parse_numerical_param(ct, *dptr,
						   matchoff + matchlen,
						   *datalen, "expires=",
						   NULL, NULL, &c_expires);
		if (ret < 0)
				...
				const char **dptr, unsigned int *datalen)
{
	...
	if (ct_cisco_sip_get_header(ct, *dptr, 0, *datalen, SIP_HDR_CSEQ,
			      &matchoff, &matchlen) <= 0)
		return NF_DROP;
			       ...
			       const char **dptr, unsigned int *datalen)
{
	 ...
	 * the correct port.
	 */
	if (ct_cisco_sip_parse_header_uri(ct, *dptr, NULL, *datalen,
...
static int process_sip_msg(struct sk_buff *skb, struct nf_conn *ct,
			   ...
			   unsigned int *datalen)
{
	typeof(nf_nat_cisco_sip_hook) nf_nat_sip;
	int ret;
	...
	if (ret == NF_ACCEPT && ct->status & IPS_NAT_MASK) {
		nf_nat_sip = rcu_dereference(nf_nat_cisco_sip_hook);
			...
			struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	int ret = NF_ACCEPT;
	bool term;
	typeof(nf_nat_cisco_sip_seq_adjust_hook) nf_nat_sip_seq_adjust;
	...
	while (1) {
		if (ct_cisco_sip_get_header(ct, dptr, 0, datalen,
	...
	if (ret == NF_ACCEPT && ct->status & IPS_NAT_MASK) {
		nf_nat_sip_seq_adjust = rcu_dereference(nf_nat_cisco_sip_seq_adjust_hook);

			...
			enum sip_header_types type)
{
	...
	if (ct_cisco_sip_parse_header_uri(ct, *dptr, NULL, *datalen, type, NULL,
			       ...
			       const char **dptr, unsigned int *datalen)
{
	...
	if (strnicmp(*dptr, "SIP/2.0", strlen("SIP/2.0")) != 0) {
		if (ct_cisco_sip_parse_request(ct, *dptr, *datalen,
					 &matchoff, &matchlen,
					 &addr, &port) > 0 &&
	...
	if (ct_cisco_sip_parse_header_uri(ct, *dptr, NULL, *datalen,
		 ...
		 * the reply. */
		if (ct_cisco_sip_parse_address_param(ct, *dptr, matchend, *datalen,
					       "maddr=", &poff, &plen,
					       &addr) > 0 &&
		 ...
		 * from which the server received the request. */
		if (ct_cisco_sip_parse_numerical_param(ct, *dptr, matchend, *datalen,
						 "rport=", &poff, &plen,
						 &n) > 0 &&
...
next:
	...
	coff = 0;
	in_header = 0;
	while (ct_cisco_sip_parse_header_uri(ct, *dptr, &coff, *datalen,
			      ...
			      const char **dptr, unsigned int *datalen)
{
	...
	if (ct_cisco_sip_get_header(ct, *dptr, 0, *datalen, SIP_HDR_CONTENT_LENGTH,
			      &matchoff, &matchlen) <= 0)
		return 0;
			     ...
			     char *buffer, int buflen)
{
	...
	if (ct_cisco_sip_get_sdp_header(ct, *dptr, sdpoff, *datalen, type, term,
				  &matchoff, &matchlen) <= 0)
		return -ENOENT;
	return mangle_packet(skb, dataoff, dptr, datalen, matchoff, matchlen,
...
{
	RCU_INIT_POINTER(nf_nat_cisco_sip_hook, NULL);
	RCU_INIT_POINTER(nf_nat_cisco_sip_seq_adjust_hook, NULL);
	RCU_INIT_POINTER(nf_nat_cisco_sip_expect_hook, NULL);
	RCU_INIT_POINTER(nf_nat_cisco_sdp_addr_hook, NULL);
	RCU_INIT_POINTER(nf_nat_cisco_sdp_port_hook, NULL);
	RCU_INIT_POINTER(nf_nat_cisco_sdp_session_hook, NULL);
	RCU_INIT_POINTER(nf_nat_cisco_sdp_media_hook, NULL);
...
{
	BUG_ON(nf_nat_cisco_sip_hook != NULL);
	BUG_ON(nf_nat_cisco_sip_seq_adjust_hook != NULL);
	BUG_ON(nf_nat_cisco_sip_expect_hook != NULL);
	BUG_ON(nf_nat_cisco_sdp_addr_hook != NULL);
	BUG_ON(nf_nat_cisco_sdp_port_hook != NULL);
	BUG_ON(nf_nat_cisco_sdp_session_hook != NULL);
	BUG_ON(nf_nat_cisco_sdp_media_hook != NULL);
	RCU_INIT_POINTER(nf_nat_cisco_sip_hook, ip_nat_sip);
	RCU_INIT_POINTER(nf_nat_cisco_sip_seq_adjust_hook, ip_nat_sip_seq_adjust);
	RCU_INIT_POINTER(nf_nat_cisco_sip_expect_hook, ip_nat_sip_expect);
	RCU_INIT_POINTER(nf_nat_cisco_sdp_addr_hook, ip_nat_sdp_addr);
	RCU_INIT_POINTER(nf_nat_cisco_sdp_port_hook, ip_nat_sdp_port);
	RCU_INIT_POINTER(nf_nat_cisco_sdp_session_hook, ip_nat_sdp_session);
	RCU_INIT_POINTER(nf_nat_cisco_sdp_media_hook, ip_nat_sdp_media);

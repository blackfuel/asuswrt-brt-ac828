...
uint32_t flash_onfi_probe(struct msm_nand_chip *chip)
{
	...
	while (cmd_set_count-- > 0) {
		...
		if (dma_buffer->data.flash_status & 0x110) {
			pr_notice("MPU/OP error (0x%x) during "
					"ONFI probe\n",
					dma_buffer->data.flash_status);
			err = -EIO;
			break;
		...
		if (cmd_set_count) {
			...
			if (onfi_param_page_ptr->parameter_page_signature !=
				...
				pr_notice("ONFI probe : Found a non"
			...
			if (crc_chk_count >= ONFI_PARAM_INFO_LENGTH
					/ ONFI_PARAM_PAGE_LENGTH) {
				pr_notice("ONFI probe : CRC Check "
				...
				pr_notice("ONFI probe : Found an ONFI "
					"compliant device %s\n",
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	unsigned cwperpage;
	uint8_t *orig_datbuf = NULL, *orig_oobbuf = NULL;
	size_t orig_len = 0, orig_ooblen = 0;
#if VERBOSE
	pr_info("================================================="
			"================\n");
	...
	if (ops->mode == MTD_OPS_RAW) {
		/* convert nandwrite style to msm_nand style.
		 * copy ops->datbuf and ops->oobbuf to a single buffer.
		 */
		orig_datbuf = ops->datbuf;
		orig_oobbuf = ops->oobbuf;
		orig_len = ops->len;
		orig_ooblen = ops->ooblen;
		ops->datbuf = chip->bounce_buf;
		if (!orig_datbuf)
...
err_dma_map_oobbuf_failed:
	...
	if (ops->mode == MTD_OPS_RAW) {
		if (orig_datbuf && orig_len)
			memcpy(orig_datbuf, ops->datbuf, orig_len);
		if (orig_oobbuf && orig_ooblen)
			memcpy(orig_oobbuf, ops->datbuf + mtd->writesize, orig_ooblen);
		...
		if (orig_ooblen) {
			ops->len -= orig_ooblen;
			ops->retlen -= orig_ooblen;
			ops->oobretlen = orig_ooblen;
		} else if (!orig_ooblen) {
			ops->len -= mtd->oobsize;
			ops->retlen -= mtd->oobsize;
			ops->oobretlen = 0;
		}
	}

exit__msm_nand_read_oob:
	if (ops->mode == MTD_OPS_RAW) {
		ops->datbuf = orig_datbuf;
		ops->len = orig_len;
		ops->oobbuf = orig_oobbuf;
		ops->ooblen = orig_ooblen;
	}
...
msm_nand_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	...
	if (ops->mode == MTD_OPS_RAW) {
		...
		orig_len = ops->len;
		orig_ooblen = ops->ooblen;
		memcpy(chip->bounce_buf, ops->datbuf, ops->len);
		ops->datbuf = chip->bounce_buf;

		if (ops->oobbuf) {
			memcpy(chip->bounce_buf + ops->len, ops->oobbuf, ops->ooblen);
			ops->len += ops->ooblen;
			ops->oobbuf = NULL;
			ops->ooblen = 0;
		}
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	if (!(chip->bounce_buf= kmalloc(mtd->writesize + mtd->oobsize + 32, GFP_KERNEL|GFP_DMA))) {
		pr_err("%s: allocate bounce buffer fail\n", __func__);
		return -ENOMEM;
	}
...
static int setup_asus_mtd_device(struct msm_nand_info *info)
{
	int err;
	struct mtd_info *mtd = &info->mtd;

#if defined(CONFIG_MTD_UBI)
	/* Disable write in Bootloader area. */
	msm_nand_partitions[0].mask_flags |= MTD_WRITEABLE;

	printk("MTD_BOOT_PART_SIZE %x MTD_CONFIG_PART_SIZE %x msm_nand_partitions[0].size %lx mtd->erasesize %lx\n",
		MTD_BOOT_PART_SIZE, MTD_CONFIG_PART_SIZE, (unsigned long)msm_nand_partitions[0].size, (unsigned long)mtd->erasesize);
	msm_nand_partitions[1].offset = ALIGN(msm_nand_partitions[0].size + MTD_CONFIG_PART_SIZE, mtd->erasesize);
	msm_nand_partitions[1].size = mtd->size - msm_nand_partitions[1].offset;
#else	/* !CONFIG_MTD_UBI */
#error FIXME
	...
	if (rfs_offset != 0) {
		msm_nand_partitions[4].offset = MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + rfs_offset;
		msm_nand_partitions[4].mask_flags |= MTD_WRITEABLE;
		if (mtd->size > 0x800000) {
			msm_nand_partitions[3].size = mtd->size - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE) - MTD_JFFS2_PART_SIZE;
			msm_nand_partitions[4].size = mtd->size - msm_nand_partitions[4].offset - MTD_JFFS2_PART_SIZE;
			msm_nand_partitions[5].offset = mtd->size - MTD_JFFS2_PART_SIZE;
		} else {
			msm_nand_partitions[3].size = mtd->size - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE);
			msm_nand_partitions[4].size = mtd->size - msm_nand_partitions[4].offset;
			msm_nand_partitions[5].name = msm_nand_partitions[6].name;
			msm_nand_partitions[5].size = msm_nand_partitions[6].size;
			msm_nand_partitions[5].offset = msm_nand_partitions[6].offset;
		}
		printk(KERN_NOTICE "partion 3: %x %x\n", (unsigned int)msm_nand_partitions[3].offset, (unsigned int)msm_nand_partitions[3].size);
		printk(KERN_NOTICE "partion 4: %x %x\n", (unsigned int)msm_nand_partitions[4].offset, (unsigned int)msm_nand_partitions[4].size);
	}
#endif

	/* Register the partitions */
	err = mtd_device_register(mtd, msm_nand_partitions, ARRAY_SIZE(msm_nand_partitions));
...
{
#if 1
	return setup_asus_mtd_device(info);
#else
	int i, err;

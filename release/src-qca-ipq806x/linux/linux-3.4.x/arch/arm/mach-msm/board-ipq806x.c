...
#if defined(CONFIG_SERIAL_MSM_HS_GSBI2)
	&ipq806x_device_uart_gsbi2,
#else
	&ipq806x_device_uart_gsbi4,
#endif
...
static void nss_gmac_init(void)
{
...
#endif	/* ! (CONFIG_SWITCH_RTL8370M_PHY_QCA8033_X2 || CONFIG_SWITCH_RTL8370MB_PHY_QCA8033_X2) */
	...
	if (machine_is_ipq806x_ap148() || machine_is_ipq806x_ap145() ||
...
#if defined(CONFIG_SWITCH_RTL8370M_PHY_QCA8033_X2)
		/* BRT-AC828 SR1~3 */
		mdata = (struct mdio_gpio_platform_data *)ipq806x_mdio1_device.dev.platform_data;
		mdata->mdc = 11;
		mdata->mdio = 10;
		mdata->phy_mask = 0;
		platform_device_register(&ipq806x_mdio1_device);

		mdiobus_register_board_info(ipq806x_ap148_mdio1_info, IPQ806X_MDIO_BUS_MAX);

		/* GMAC0: 2.5V RGMII,	LAN	RTL8370M-GR RG1
		 * GMAC1: 1.8V RGMII,	LAN	RTL8370M-GR RG0
		 * GMAC2: SGMII1,	WAN0	QCA8033 PHY
		 * GMAC3: SGMII2,	WAN1	QCA8033 PHY
		 */
		pdata = (struct msm_nss_gmac_platform_data *)nss_gmac_0.dev.platform_data;
		...
		/* GMAC0: 2.5V RGMII,	WAN0	QCA8033 PHY
		 * GMAC1: SGMII0,	LAN	RTL8370MB
		 * GMAC2: SGMII1,	LAN	RTL8370MB
		 * GMAC3: SGMII2,	WAN1	QCA8033 PHY
		 */
		pdata = (struct msm_nss_gmac_platform_data *)nss_gmac_0.dev.platform_data;
		pdata->phy_mdio_addr = 5;
		...
		platform_device_register(&nss_gmac_2);
		platform_device_register(&nss_gmac_3);
#elif !defined(CONFIG_MACH_IPQ806X_AP148) || defined(CONFIG_SWITCH_QCA8337N)
		/* non-AP148 boards or AP148 with QCA8337N switch */
...
#define WAN0_MAC_OFFSET		0x5006	/* 2G MAC */
#define LAN_MAC_OFFSET		0x1006	/* 5G MAC */
#endif
...
void __init ipq_nss_get_mac_addr(struct mtd_info *mtd, int id, struct msm_nss_gmac_platform_data *pdata)
{
	...
	loff_t eeprom_off = 0;
	uint8_t *mac = pdata->mac_addr, delta = 0;
	uint8_t temp_mac_src[2][sizeof(pdata->mac_addr)] = {
		{ 0x02, 0xaa, 0xbb, 0xcc, 0xdd, 0xe0 },
		{ 0x02, 0xaa, 0xbb, 0xcc, 0xdd, 0xe4 },
	}, *m = temp_mac_src[0];
	 ...
	 * WAN0 MAC = WAN_MAC + 1
	 * WAN1 MAC = WAN_MAC + 2
	 * If bonding is enabled, MAC address of all bonding
	 * slave interfaces are equal to each other.
	 */
#if defined(CONFIG_SWITCH_RTL8370M_PHY_QCA8033_X2)
	/* BRT-AC828 SR1~3 */
	if (id == 0 || id == 1) {
		/* LAN0, LAN1 */
		eeprom_off = LAN_MAC_OFFSET & EEPROM_OFFSET_MASK;
		off = LAN_MAC_OFFSET - eeprom_off;
		delta = id + 1;
	} else if (id == 2 || id == 3) {
		/* WAN0, WAN1 */
		eeprom_off = WAN0_MAC_OFFSET & EEPROM_OFFSET_MASK;
		off = WAN0_MAC_OFFSET - eeprom_off;
		m = temp_mac_src[1];
		delta = id - 1;
	} else {
		printk("%s: Unknown MAC[%d]\n", __func__, id);
		return;
	}
#elif defined(CONFIG_SWITCH_RTL8370MB_PHY_QCA8033_X2)
	/* BRT-AC828 SR4 or above */
	...
	if ((ret = mtd_read(mtd, eeprom_off, QC98XX_EEPROM_SIZE_LARGEST, &len, eeprom)) ||
	    (len != QC98XX_EEPROM_SIZE_LARGEST)) {
	...
	if (!qc98xx_verify_checksum(eeprom))
		...
		csum_fail = 1;
		m = temp_mac_src[0];
		printk("%s: Verify EEPROM at %lx fail\n", __func__, (unsigned long)eeprom_off);
	}

	memcpy(mac, m, sizeof(pdata->mac_addr));
	if (csum_fail)
		mac[5] += id;
	else
		mac[5] += delta;
...
void __init ipq_nss_get_mac_addr(struct mtd_info *mtd, int id, struct msm_nss_gmac_platform_data *pdata)
{
	...
	uint8_t temp_mac_src[sizeof(pdata->mac_addr)] = { 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xe0 }, *m = temp_mac_src;
	...
	if (csum_fail)
		mac[5] += id;
	else if (!csum_fail && id == 3)
		mac[5]++;

	printk("%s: MAC[%d]: %02x:%02x:%02x:%02x:%02x:%02x\n", __func__, id,
			mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
#else	/* !CONFIG_ASUS_BRTAC828 && !CONFIG_QCA_AP148 */
inline void __init ipq_nss_get_mac_addr(struct mtd_info *mtd, int id,
					struct msm_nss_gmac_platform_data *pdata)
{
	int ret, len;
...
int wifi_board_data_read(loff_t from, size_t len, size_t *retlen, u_char *buf)
{
	...
	r = mtd_read(mtd, from, len, retlen, buf);
	put_mtd_device(mtd);

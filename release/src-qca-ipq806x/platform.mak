...
IPQ807X_MODEL_LIST := $(addprefix _,$(addsuffix _,GT-AXY16000))
...
ifeq ($(QGCC483_MUSL32),y)
export CROSS_COMPILE := arm-openwrt-linux-muslgnueabi-
export READELF := arm-openwrt-linux-muslgnueabi-readelf
else ifeq ($(QGCC483_MUSL64),y)
export CROSS_COMPILE := aarch64-openwrt-linux-musl-
export READELF := aarch64-openwrt-linux-musl-readelf
else
...
define platformRouterOptions
	...
	if [ "$(QCA)" = "y" ]; then \
		...
		if [ "$(QGCC483_MUSL32)" = "y" -o "$(QGCC483_MUSL64)" = "y" ] ; then \
	...
	if [ "$(QCA)" = "y" ]; then \
		...
		if [ "$(IPQ807X)" = "y" ] ; then \
			...
			if [ -n "$(findstring $(_BUILD_NAME_),$(IPQ807X_MODEL_LIST))" ] ; then \
				...
				sed -i "/CONFIG_NETWORK_PHY_TIMESTAMPING\>/d" $(1); \
				echo "CONFIG_NETWORK_PHY_TIMESTAMPING=y" >>$(1); \

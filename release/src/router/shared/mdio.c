 ...
 * @return:
 * 	0:	register value or error
 *  otherwise:	register value
 */
...
int mdio_phy_speed(unsigned int phy)
{
	...
	bmsr = read_phy_reg(phy, MII_BMSR);
	if (!(bmsr & BMSR_LSTATUS))
		return 0;

	bmcr = read_phy_reg(phy, MII_BMCR);
	if (!(bmcr & BMCR_ANENABLE)) {
		if (bmcr & BMCR_SPEED1000)
			return 1000;
		else if (bmcr & BMCR_SPEED100)
	...
	advert = read_phy_reg(phy, MII_ADVERTISE);
	lkpar = read_phy_reg(phy, MII_LPA);
	if (!(advert & lkpar)) {
		dbg("%s: phy 0x%x: autonegotiation failed\n", __func__, phy);
		return 0;
	}

	bmcr2 = read_phy_reg(phy, MII_CTRL1000);
	lpa2 = read_phy_reg(phy, MII_STAT1000);
	mask = advert & lkpar;
	mask2 = bmcr2 & (lpa2>>2);
	if (mask2 & (ADVERTISE_1000FULL | ADVERTISE_1000HALF))
		return 1000;
...
int aqr_phy_speed(unsigned int phy)
{
	...
	xs_status = read_phy_reg(phy, 0x4004E812);
	...
	prvs1 = read_phy_reg(phy, 0x4001E800);
	...
	avs1 = read_phy_reg(phy, 0x4007C800);
	speed = (avs1 >> 1) & 7;

...
#include <libnt.h>
extern call_notify_center(int sort, int event);
#endif
...
int Nbns_query(unsigned char *src_ip, unsigned char *dest_ip, P_CLIENT_DETAIL_INFO_TABLE p_client_detail_info_tab, int i)
{
	...
	while(1)
	{
		...
		if( recvlen > 0 ) {
			...
			else if(((nbns_response->flags[0]>>4) == 8) && (nbns_response->number_of_names > 0)
...
void
get_name_from_dhcp_lease(unsigned char *mac, char *dev_name, int size)
{
	FILE *fp;
...
int FindAllApp(unsigned char *src_ip, P_CLIENT_DETAIL_INFO_TABLE p_client_detail_info_tab, int i)
{
...
#if 0
	//check if Asus_discovery already found, no need to search again
	if(!(p_client_detail_info_tab->device_flag[i] & (1<<FLAG_HTTP)))
	{
		//http service detect
	...
	if(!ret) {
		...
		p_client_detail_info_tab->device_flag[i] |= (1<<FLAG_PRINTER);
		file_unlock(lock);
	}
	else {
		ret = raw9100(dest_ip);
	...
	if(ret) {
		...
		p_client_detail_info_tab->device_flag[i] |= (1<<FLAG_ITUNE);
	}
	else {
	...
	if(p_client_detail_info_tab->type[i]==0) {
		if( ctrlpt(dest_ip) )//UPNP detect
		{
			...
			if(strcmp("",description.modelname) &&
				...
				strlcpy(p_client_detail_info_tab->device_name[i], description.modelname, sizeof(p_client_detail_info_tab->device_name[i]));
			}
			file_unlock(lock);
		}
		else 
	...
	if(p_client_detail_info_tab->type[i]==0)
	{
		...
		memcpy(des_hostname, NetBIOS_name, sizeof(des_hostname));
	...
	if(!strcmp("", p_client_detail_info_tab->device_name[i])) {
		get_name_from_dhcp_lease(p_client_detail_info_tab->mac_addr[i],
				p_client_detail_info_tab->device_name[i], sizeof(p_client_detail_info_tab->device_name[i]));
		fixstr(p_client_detail_info_tab->device_name[i]);
	...
	if(strstr(name2lower, filter)) {
		...
		if(p_client_detail_info_tab->vendor_name[x][0] != '\0' && strlen(p_client_detail_info_tab->vendor_name[x]) <= 22) {
			sprintf(p_client_detail_info_tab->device_name[x], 
				"android(%s)\0", p_client_detail_info_tab->vendor_name[x]);
		}
		else
	...
	if ((fp = fopen("/var/lib/misc/dnsmasq.leases", "r"))) {
		...
		while ((next = fgets(line, sizeof(line), fp)) != NULL) {
			if (vstrsep(next, " ", &expire, &mac, &ip, &name) == 4) {
				...
				{
					strlcpy(p_client_detail_info_tab->device_name[p_client_detail_info_tab->detail_info_num], name, 
						sizeof(p_client_detail_info_tab->device_name[p_client_detail_info_tab->detail_info_num]));
					extern ac_state *acType;
					if(!p_client_detail_info_tab->type[p_client_detail_info_tab->detail_info_num]) {
						device2lower = strdup(p_client_detail_info_tab->device_name[p_client_detail_info_tab->detail_info_num]);
						toLowerCase(device2lower);
						if((typeID = full_search(acType, device2lower))) {
							p_client_detail_info_tab->type[p_client_detail_info_tab->detail_info_num] = typeID;
							NMP_DEBUG("DHCP: Find type :%d!\n", typeID);
						}
						free(device2lower);
					}
					Device_name_filter(p_client_detail_info_tab, p_client_detail_info_tab->detail_info_num);
#ifdef RTCONFIG_NOTIFICATION_CENTER
					if(p_client_detail_info_tab->type[p_client_detail_info_tab->detail_info_num] == 1)
						call_notify_center(FLAG_SAMBA_INLAN, HINT_SAMBA_INLAN_EVENT);
					if(p_client_detail_info_tab->type[p_client_detail_info_tab->detail_info_num] == 7)
						call_notify_center(FLAG_XBOX_PS, HINT_XBOX_PS_EVENT);
					if(p_client_detail_info_tab->type[p_client_detail_info_tab->detail_info_num] == 27)
						call_notify_center(FLAG_UPNP_RENDERER, HINT_UPNP_RENDERER_EVENT);
					if(p_client_detail_info_tab->type[p_client_detail_info_tab->detail_info_num] == 6)
						call_notify_center(FLAG_OSX_INLAN, HINT_OSX_INLAN_EVENT);
#endif
				}
				//ipMethod: DHCP
				if (!strcmp(ipaddr, ip))
					strlcpy(p_client_detail_info_tab->ipMethod[p_client_detail_info_tab->detail_info_num], "DHCP", 
		...
		while ((b = strsep(&nvp, "<")) != NULL) {
			...
			if ((vstrsep(b, ">", &mac, &ip) == 2) && (strlen(ip) > 0)) {
				if (!strcmp(ipaddr, ip)) {
					//strlcpy(p_client_detail_info_tab->device_name[p_client_detail_info_tab->detail_info_num], name, 
					//	sizeof(p_client_detail_info_tab->device_name[p_client_detail_info_tab->detail_info_num]));
					strlcpy(p_client_detail_info_tab->ipMethod[p_client_detail_info_tab->detail_info_num], "Manual", 
						sizeof(p_client_detail_info_tab->ipMethod[p_client_detail_info_tab->detail_info_num]));
				}
			}
		}

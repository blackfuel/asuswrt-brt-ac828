...
{
        ARP_HEADER * arp;
	uint32_t my_ipaddr_ne = htonl(my_ipaddr_he.s_addr);
	char raw_buffer[46];
	...
	bzero(raw_buffer, 46);
        ...
        memcpy(arp->source_ipaddr, &my_ipaddr_ne, 4);
...
{
	int arp_sockfd, arp_getlen, i, max_scan_count;
	...
	//client tables
	_dprintf("networkmap: size of shared memory: %u Bytes. (Max. number of clients: %u)\n", sizeof(CLIENT_DETAIL_INFO_TABLE), MAX_NR_CLIENT_LIST);
	...
	NMP_DEBUG(" Test networkmap trigger flag >>> %d!\n", TRIGGER_FLAG);	
		...
		fullscan:
			...
			NMP_DEBUG("Starting full scan!\n");
				...
				int x;
				for (x = 0; x < ARRAY_SIZE(p_client_detail_info_tab->exist); x++)
		    ...
		    if (scan_count < max_scan_count && (scan_ipaddr_he != my_ipaddr_he.s_addr)) {
                        sent_arppacket(arp_sockfd, (unsigned char*) &scan_ipaddr_ne);
		    }         
		...
		{ 	
			...
			{
#if defined(RTCONFIG_BWDPI) || defined(RTCONFIG_BWDPI_DEP)
				printf(" %d.%d.%d.%d /%02X:%02X:%02X:%02X:%02X:%02X/%s/%s/%d/%d/%d/%d/%s/%s/%s\n",
...
*/
		    ...
		    arp_srcipaddr_ne = *(long*)arp_ptr->source_ipaddr;
                    if (netaddr_ne == (arp_srcipaddr_ne & netmask_ne.s_addr)) {
			msg_type = ntohs(arp_ptr->message_type);

			if( //ARP packet to router
			   (msg_type == 0x02 &&								// ARP response
			    !memcmp(arp_ptr->dest_ipaddr, &router_addr_ne.sin_addr.s_addr, 4) &&	// dest IP
			   ...
			   (msg_type == 0x01 &&								// ARP request
				...
				if((ip_dup == 0) && (mac_dup == 0)) {
				...
				else if( (scan_count >= max_scan_count) && ((ip_dup != 0) && (mac_dup == 0)) ) {
					...
					p_client_detail_info_tab->detail_info_num = i;
                                        ...
                                        #endif
					FindAllApp((unsigned char*)&router_addr_ne.sin_addr.s_addr, p_client_detail_info_tab);
				...
				if (p_client_detail_info_tab->ip_mac_num < ARRAY_SIZE(p_client_detail_info_tab->ip_addr)) {
					lock = file_lock("networkmap");
					...
					dbg("Number of scanned clients (%d) is larger than expected!\n",
						p_client_detail_info_tab->ip_mac_num);
				}
                	    }
			}
                        	...
                        	if( i==p_client_detail_info_tab->ip_mac_num ) //Find a new IP or table is empty! Send an ARP request.
				{
					...
					if(memcmp(&router_addr_ne.sin_addr.s_addr, arp_ptr->source_ipaddr, 4))
	...
	*/
		...
		nvram_set("networkmap_status", "1");
		FindAllApp((unsigned char*)&router_addr_ne.sin_addr.s_addr, p_client_detail_info_tab, p_client_detail_info_tab->detail_info_num);
		lock = file_lock("networkmap");
		FindHostname(p_client_detail_info_tab);

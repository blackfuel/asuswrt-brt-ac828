...
ej_wl_get_guestnetwork(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	foreach(word2, nvram_safe_get(strcat_r(prefix, "vifnames", tmp)), next2) {
		...
		ret += websWrite(wp, "\", \"");
		ret += webWriteNvram2(wp, strcat_r(word2, "_guest_num", tmp));	// gn_array[][18]
		ret += websWrite(wp, "\"]");
	}
	ret += websWrite(wp, "]");
	return ret;
...
static void update_subnet_rulelist(void){
	char *nv, *nvp, *b;
	char *gateway, *netmask, *dhcp_enable, *dhcp_start, *dhcp_end, *dhcp_lease;
	char *lan_domain, *dhcp_dns1, *dhcp_wins, *dhcp_static, *dhcp_staticlist;
	int count = 0, update = 0;
	char *subnet_rulelist;

	nv = nvp = strdup(nvram_safe_get("subnet_rulelist"));
	subnet_rulelist = malloc(sizeof(nv) + 512);
	while (nv && (b = strsep(&nvp, "<")) != NULL) {
		if (vstrsep(b, ">", &gateway, &netmask, &dhcp_enable, &dhcp_start, &dhcp_end, &dhcp_lease,
			        &lan_domain, &dhcp_dns1, &dhcp_wins, &dhcp_static, &dhcp_staticlist) < 11)
			continue;
		count++;

		if(count == 1){
			if(nvram_invmatch("dhcp_gateway_x", gateway) || nvram_invmatch("lan_netmask", netmask) || nvram_invmatch("dhcp_enable_x", dhcp_enable) ||
			   nvram_invmatch("dhcp_start", dhcp_start)  || nvram_invmatch("dhcp_end", dhcp_end) || nvram_invmatch("dhcp_dns1_x", dhcp_dns1) ||
			   nvram_invmatch("dhcp_wins_x", dhcp_wins) ){
			   	sprintf(subnet_rulelist, "<%s>%s>%s>%s>%s>%s>%s>%s>%s>%s>%s", nvram_safe_get("dhcp_gateway_x"),
						...
						nvram_safe_get("dhcp_dns1_x"), nvram_safe_get("dhcp_wins_x"), dhcp_static, dhcp_staticlist);
				update = 1;
			}
			break;
		}
	...
	if(update == 1){
		if(nvp != NULL){
			strcat(subnet_rulelist, "<");
			strcat(subnet_rulelist, nvp);
		}
		nvram_set("subnet_rulelist", subnet_rulelist);
	}

	free(subnet_rulelist);
	free(nv);
}
#endif
...
static int ej_update_variables(int eid, webs_t wp, int argc, char_t **argv) {
	...
	int sleep1 = 0, sleep2 = 0, bss_sleep = 0;
	int delta1 = 0;
#endif
...
#define HEADER_LEN (8)
	...
	if (upgrade_err) /* 0: legal image, 1: illegal image */
		goto err;
#endif
...
//	int disk_num = 0;
	...
	if(!strcmp(disk_port, "all")){
		result = eval("/sbin/ejusb", "-1", "0", "-u" , "1");
	}
	else{
		result = eval("/sbin/ejusb", disk_port, "0", "-u", "1");
	}
...
loopagain:
			...
			if (from_app == 0) {
				...
				ret += websWrite(wp, "%c\"%s_rx\":\"0x%lx\",\"%s_tx\":\"0x%lx\"", comma, "WIRED", wired_all_rx, "WIRED", wired_all_tx);
			}
			comma = ',';
...
static void ASN1_TimeToTM(ASN1_TIME* time, struct tm *t)
{
	...
	if (time->type == V_ASN1_UTCTIME) {
		t->tm_year = (data[i] - '0') * 10 + (data[i+1] - '0');
		i += 2;
		if(t->tm_year < 70)
			t->tm_year += 100;
	}
	else if (time->type == V_ASN1_GENERALIZEDTIME) {
		t->tm_year = (data[i] - '0') * 1000 + (data[i+1] - '0') * 100 + (data[i+2] - '0') * 10 + (data[i+3] - '0');
		i += 4;
		t->tm_year -= 1900;
	}
...
ej_httpd_cert_info(int eid, webs_t wp, int argc, char **argv)
{
	...
	//use PEM format
	PEM_read_X509(fp, &x509data, NULL, NULL);	//d2i_X509_fp(fp, &x509data);
	fclose(fp);
	if(x509data == NULL){
		websWrite(wp, "{\"issueTo\":\"\",\"issueBy\":\"\",\"from\":\"\",\"expire\":\"\"}");
		return FALSE;

...
if [ "$modem_type" == "tty" -o "$modem_type" == "qmi" -o "$modem_type" == "mbim" -o "$modem_type" == "gobi" ] || [ "$usb_gobi2" == "1" ]; then
	...
	if [ "$modem_enable" == "2" ]; then
		...
		if [ "$usb_gobi2" == "1" ]; then
			...
			at_ret=`$at_lock /usr/sbin/modem_at.sh '+CGDCONT=1,"'$pdp_str'","'$modem_apn'"' 2>&1`
			ret=`echo -n $at_ret |grep "OK" 2>/dev/null`
			if [ -z "$ret" ]; then
			...
			if [ "$modem_user" != "" -o "$modem_pass" != "" ]; then
				...
				#fi
				#at_ret=`$at_lock /usr/sbin/modem_at.sh '$QCPDPP=1,'$modem_authmode',"'$modem_pass'","'$modem_user'"' 2>&1`
				at_ret=`$at_lock /usr/sbin/modem_at.sh '$PPP='$modem_user','$modem_pass 2>&1`
				ret=`echo -n $at_ret |grep "OK" 2>/dev/null`
				if [ -z "$ret" ]; then
		...
		if [ "$modem_vid" == "4817" -a "$modem_pid" == "5132" ]; then
			...
			while [ $tries -le 30 ] && [ -z "$ret" ]; do
				...
				at_ret=`$at_lock /usr/sbin/modem_at.sh '+CGATT?' 2>&1`
	...
	if [ "$Dev3G" == "Docomo_dongles" ] || [ "$is_Docomo" -eq "1" ]; then
		...
		at_ret=`$at_lock /usr/sbin/modem_at.sh '+COPS?' 2>&1`
		ret=`echo -n "$at_ret" |grep "OK" 2>/dev/null`
		if [ -n "$ret" ]; then
			...
			if [ "$modem_roaming" != "1" ]; then
				...
				$at_lock /usr/sbin/modem_at.sh '+COPS=0' "$modem_reg_time"
				#at_ret=`$at_lock /usr/sbin/modem_at.sh '+COPS?' 2>&1`
				#ret=`echo -n "$at_ret" |grep "+COPS:" |awk '{print $2}' |awk 'BEGIN{FS=","}{print $1}' 2>/dev/null`
				...
				$at_lock /usr/sbin/modem_at.sh '+COPS=0' "$modem_reg_time"
				#at_ret=`$at_lock /usr/sbin/modem_at.sh '+COPS=0' "$modem_reg_time" 2>&1`
				#ret=`echo -n $at_ret |grep "OK" 2>/dev/null`
				#if [ -z "$ret" ]; then
	...
	if [ "$modem_vid" == "8193" ];then
		...
		#at_ret=`$at_lock /usr/sbin/modem_at.sh '+BMHPLMN' 2>&1`
		...
		if [ "$usb_gobi2" == "1" ]; then
			at_ret=`$at_lock /usr/sbin/modem_at.sh '+CAUTOCONNECT=1' 2>&1`
			ret=`echo -n $at_ret |grep "OK" 2>/dev/null`
			if [ -z "$ret" ]; then

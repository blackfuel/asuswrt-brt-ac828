...
*qca_ol_params[] = {
	...
	"qca9888_20_targ_clk", "lteu_support", "preCACEn",
#if defined(RTCONFIG_GLOBAL_INI)
	/* SPF10 ES QSDK */
	"enable_eapol_minrate", "set_eapol_minrate_ac",
	"twt_enable", "interCACChan",
#endif
#endif
	NULL
},
...
#if defined(RTCONFIG_WIFI_QCN5024_QCN5054)
	{ "smart_antenna", 0, 0, 0, NULL, NULL },
#endif
#if defined(RTCONFIG_WIGIG)
	{ "wil6210", 0, 0, 0, NULL, NULL },
#endif
};
 ...
 * For global.ini, this function borrows buffer provided by caller,
 * add parameters there, pass them to __update_ini_file(), and then
 * restore related pointers.
 */
static void qca_ol_param_hook(char ***pv, char **ps, int *plen)
{
	char **old_pv __attribute__((unused)), *old_ps __attribute__((unused));
	int olcfg __attribute__((unused)) = nss_wifi_offloading();
	int old_plen __attribute__((unused));
#if defined(RTCONFIG_GLOBAL_INI)
	const int global_ini = 1;
#else
	const int global_ini = 0;
#endif
...
#if defined(RTCONFIG_GLOBAL_INI)
	*(*pv)++ = NULL;
	__update_ini_file(GLOBAL_INI, old_pv);
	*pv = old_pv;
	*ps = old_ps;
	*plen = old_plen;
#endif
...
static void wifi_3_0_post_hook(void)
{
	...
	update_ini_file(GLOBAL_INI, hw_mode_id_str);
}
#endif
...
#if defined(RTCONFIG_SOC_IPQ8074)
		if (!strcmp(*qmod, "qca-ssdk") && nvram_match("sfp+_force_on", "1")) {
			*v++ = "xgmac_autodet=N";
		}
#endif
#if defined(RTCONFIG_SOC_IPQ40XX)
...
int switch_exist(void)
{
	...
	for (p = &phy_id_lists[0]; p->phy >= 0; ++p) {
		...
		r = ((read_phy_reg(p->phy, id_reg_addr) & 0xFFFF) << 16) | (read_phy_reg(p->phy, id_reg_addr + 1) & 0xFFFF);
	...
	if (is_aqr_phy_exist()) {
		...
		if (fw == 0 || fw == 0xFFFF || build == 0 || build == 0xFFFF) {
			dbg("Can't get AQR PHY firmware version. (%x,%x)\n", fw, build);
			ret = 0;
		} else {
...
static void __load_wifi_driver(int testmode)
{
	...
	char param[512], *s = &param[0], umac_nv[64], *val, *stats_cmd;

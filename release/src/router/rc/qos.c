	...
	for (unit = WAN_UNIT_FIRST; unit < WAN_UNIT_MAX; ++unit) {
		...
		if (!unit) {
			...
			fprintf(f,
				...
				"\t$TFA parent 1: prio 6 protocol %s u32 match mark 6 0x3F flowid 1:60\n", protocol
				);
		}
#endif
		...
		for (i = 0; i < 5; ++i) { // 0~4 , 0:highest, 4:lowest
			...
			fprintf(f,
				...
				"\t$TFA parent 1: prio %d protocol ip u32 match mark %d 0x3F flowid 1:%d\n",
					i, rate, ceil,
		...
		if (bw > 0) {
			...
			for (i = 0; i < 5; ++i) { // 0~4 , 0:highest, 4:lowest
				...
				fprintf(f,
					...
					"\t$TFADL parent 2: prio %d protocol ip u32 match mark %d 0x3F flowid 2:%d\n",
						i, rate,
				...
				fprintf(f,
					"# ingress %d: %u%%\n"
					"\t$TFA parent ffff: prio %d protocol ip u32 match mark %d 0x3F"
...
static int add_bandwidth_limiter_rules(char *pcWANIF)
{
	...
	*/
	...
	fprintf(fn,
		"-A POSTROUTING -s %s -d %s -j %s 9/0x3F\n"
		"-A PREROUTING -s %s -d %s -j %s 9/0x3F\n"
	...
	while (g) {
		...
		if (addr_type == TYPE_IP){
			fprintf(fn,
				"-A POSTROUTING ! -s %s -d %s -j %s %d/0x3F\n"
				"-A PREROUTING -s %s ! -d %s -j %s %d/0x3F\n"
			...
			fprintf(fn,
				"-A PREROUTING -m mac --mac-source %s ! -d %s  -j %s %d/0x3F\n"
				, addr_new, lan_addr, action, atoi(prio)+INITIAL_MARKNUM
				);
			if(manual_return){
			fprintf(fn,
			...
			fprintf(fn,
				"-A POSTROUTING ! -s %s -m iprange --dst-range %s -j %s %d/0x3F\n"
				"-A PREROUTING -m iprange --src-range %s ! -d %s -j %s %d/0x3F\n"
				, lan_addr, addr_new, action, atoi(prio)+INITIAL_MARKNUM
				, addr_new, lan_addr, action, atoi(prio)+INITIAL_MARKNUM
				);
			if(manual_return){

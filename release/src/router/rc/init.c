...
//				nvram_set(strcat_r(prefix, "ifname", tmp), tmp2);
	...
	nvram_set(("wl0_acs_excl_chans_cfg", tmp), "");
	nvram_set(("wl1_acs_excl_chans_cfg", tmp), "");
	nvram_set(("wl2_acs_excl_chans_cfg", tmp), "");
	nvram_set(("wl0_acs_excl_chans", tmp), "");
	nvram_set(("wl1_acs_excl_chans", tmp), "");
	nvram_set(("wl2_acs_excl_chans", tmp), "");
#endif
}
...
//		nvram_set_int("led_all_gpio", 10|GPIO_ACTIVE_LOW);
	...
	case MODEL_RTAX89U:
		...
		if (nvram_match("HwId", "A") || !strlen(nvram_safe_get("HwId"))) {

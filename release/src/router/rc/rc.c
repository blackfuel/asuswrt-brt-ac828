...
static int __hotplug_dump_q6mem(void)
{
	...
	snprintf(path, sizeof(path), "/jffs/%s_%s.gz", devicename, ts);
	snprintf(dmesg_cmd, sizeof(dmesg_cmd), "dmesg > /jffs/dmesg_%s.txt", ts);
	system(dmesg_cmd);

	snprintf(gz_cmd, sizeof(gz_cmd), "gzip -c /dev/%s > %s", devicename, path);
	dbg("%s: gzip /dev/%s to %s ...\n", __func__, devicename, path);
	system(gz_cmd);
	dbg("%s: gzip /dev/%s to %s done, restart_wireless\n", __func__, devicename, path);

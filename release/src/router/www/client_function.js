...
function genClientList(){
	...
	totalClientNum.wireless_main = 0;
	totalClientNum.wireless_gn1 = 0;
	totalClientNum.wireless_gn2 = 0;
	totalClientNum.wireless_gn3 = 0;
	totalClientNum.wireless_ifnames = {}; // {all_0:0, all_1:0, all_2:0, main_0:0, main_1:0, main_2:0, gn1_0:0, gn1_1:0, gn1_2:0 ...}, interface_2g/5g: count
	var guest_group_num = gn_array_2g.length;
	for(var i = 0; i < wl_nband_title.length; i += 1) {
		totalClientNum.wireless_ifnames["all_" + i + ""] = 0;
		if(guest_group_num > 0) {
			totalClientNum.wireless_ifnames["main_" + i + ""] = 0;
			for(var j = 1; j <= guest_group_num; j += 1) {
				totalClientNum.wireless_ifnames["gn" + j + "_" + i + ""] = 0;
			}
		}
	}
	...
	for(var i=0; i<originData.wlList_2g.length; i++){
		...
		if(originData.wlList_2g[i][1] == "Yes") {
			...
			totalClientNum.wireless_ifnames["all_" + (clientList[thisClientMacAddr].isWL-1) + ""] += clientList[thisClientMacAddr].macRepeat;
		...
		if(key != "") {
			...
			if(typeof clientList[clientMac] != "undefined") {
				...
				if(clientList[clientMac].ip == "offline" && wirelessOnline) {
					...
					totalClientNum.wireless_ifnames["all_" + (clientList[clientMac].isWL-1)]--;
					if(clientList[clientMac].wlInterface != "")
						totalClientNum.wireless_ifnames[clientList[clientMac].wlInterface + "_" + (clientList[clientMac].isWL-1)]--;
				}
		...
		var updateStaInfo = function(wlLog, wlMode) {
			for(var i = 0; i < wlLog.length; i += 1) {
				...
				if(clientList[thisClientMacAddr].isOnline && (clientList[thisClientMacAddr].isWL == wlMode)) {
					...
					if(wlLog[i][4] != undefined) {
						clientList[thisClientMacAddr].wlInterface = wlLog[i][4].toLowerCase();
						totalClientNum.wireless_ifnames[clientList[thisClientMacAddr].wlInterface + "_" + (clientList[thisClientMacAddr].isWL-1) + ""] += clientList[thisClientMacAddr].macRepeat;
						switch(wlLog[i][4].toLowerCase()) {
							case "main" :
								totalClientNum.wireless_main++;
								break;
							case "gn1" :
								totalClientNum.wireless_gn1++;
								break;
							case "gn2" :
								totalClientNum.wireless_gn2++;
								break;
							case "gn3" :
								totalClientNum.wireless_gn3++;
								break;
							case "gn4" :
								totalClientNum.wireless_gn4++;
								break;
							case "gn5" :
								totalClientNum.wireless_gn5++;
								break;
							case "gn6" :
								totalClientNum.wireless_gn6++;
								break;
							case "gn7" :
								totalClientNum.wireless_gn7++;
								break;
						}
					}
				}

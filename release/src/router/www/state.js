...
var isSwMode = function(mode){
	...
	else if((sw_mode == '3' && wlc_psta == '0') || (sw_mode == '3' && wlc_psta == '')) ui_sw_mode = "ap"; // AP
...
tabtitle[0] = new Array("", "<#menu5_1_1#>", "<#menu5_1_2#>", "WDS", "<#menu5_1_4#>", "<#menu5_1_5#>", "Free Wi-Fi"/*untranslated*/, "<#menu5_1_6#>", "Airtime Fairness"/*untranslated*/);
...
if(bwdpi_support){
	if(traffic_analyzer_support){
		...
		if(traffic_limiter_support) {
			...
			tabtitle[15] = new Array("", "<#Guest_Network#>", "Free Wi-Fi"/*untranslated*/, "Captive Portal"/*untranslated*/, "Facebook Wi-Fi");
		}
		else {
...
tablink[0] = new Array("", "Advanced_Wireless_Content.asp", "Advanced_WWPS_Content.asp", "Advanced_WMode_Content.asp", "Advanced_ACL_Content.asp", "Advanced_WSecurity_Content.asp", "Advanced_Captive_Portal.asp", "Advanced_WAdvanced_Content.asp", "Advanced_Airtime_Fairness.asp");
...
if(bwdpi_support){
	if(traffic_analyzer_support){
		...
		if(traffic_limiter_support) {
			...
			tablink[15] = new Array("", "Guest_network.asp", "Captive_Portal.asp", "Captive_Portal_Advanced.asp", "Guest_network_fbwifi.asp");
		}
		else {
...
function remove_url(){
	...
	if(based_modelid == "BRT-AC828" || based_modelid == "RT-N10U"){	//MODELDEP
	...
	if(!atf_support){
		remove_menu_item("Advanced_Airtime_Fairness.asp");
	}
	...
	if(sw_mode == 2 || sw_mode == 4){
		...
		//Free Wi-Fi
		remove_menu_item("Captive_Portal.asp");
		//Captive Portal
		remove_menu_item("Captive_Portal_Advanced.asp");
		//Facebook Wi-Fi
		remove_menu_item("Guest_network_fbwifi.asp");
	}
	...
	if(!pptpd_support && !openvpnd_support && !ipsec_support){
		if(!vpnc_support){
			menuL2_title[7] = "";
			menuL2_link[7] = "";
		}
		...
		if(!pptpd_support && !openvpnd_support) {
			tablink[6][1] = "Advanced_VPN_IPSec.asp";
			menuL2_link[7] = "Advanced_VPN_IPSec.asp";
		}
	}
	...
	else if(!cp_freewifi_support) {
		remove_menu_item("Captive_Portal.asp");
	}
	else if(!cp_advanced_support) {
		remove_menu_item("Captive_Portal_Advanced.asp");
	}
}
...
function show_menu(){
	...
	if(current_url.indexOf("Advanced_VPN") == 0){
		...
		L2 = traffic_L2_dx;
		L3 = 1;
		if(current_url.indexOf("Advanced_VPNClient_Content.asp") == 0 && (pptpd_support || openvpnd_support || ipsec_support)) {
			L3 = 2;
		}
	}	
	...
	if(captivePortal_support) {
		...
		if(current_url.indexOf("Captive_Portal.asp") == 0) {
			traffic_L1_dx = 2;
			traffic_L2_dx = 16;
			traffic_L3_dx = 2;
			L1 = traffic_L1_dx;
	...
	//unset networkmap cookie
	if(current_url != "index.asp") {
		cookie.unset("wireless_unit");
...
function browser_compatibility(){
	...
	if((isFirefox || isOpera) && document.getElementById("FormTitle")){
		...
		if(current_url.indexOf("Guest_network") == 0 && !captivePortal_support)
...
function change_qos_unit(){
	FormActions("apply.cgi", "change_qos_unit", "", "");
	document.form.target = "";
	document.form.submit();
}

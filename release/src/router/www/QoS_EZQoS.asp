...
function initial(){
	...
	var qos_type = getRadioValue(document.form.qos_type);
...
function init_changeScale(){
	...
	if(dualWAN_support)
	{
		var upload2 = document.form.qos_obw1.value;
		var download2 = document.form.qos_ibw1.value;
		document.form.obw1.value = upload2/1024;
		document.form.ibw1.value = download2/1024;
	}
}
...
function submitQoS(){
	...
	if(document.form.qos_enable.value == 1){
		...
		if(qos_type != 2){
			...
			else if(qos_type == 0 && document.form.obw.value == 0){		// To check field is 0 && Traditional QoS
			...
			else if(qos_type == 0 && document.form.ibw.value == 0){		// To check field is 0 && Traditional QoS
...
function change_qos_type(value){
	...
	setRadioValue(document.form.qos_type, value);
}
...
function cancel_priority_panel() {	
	...
	setTimeout("change_qos_type(getRadioValue(document.form.qos_type));", 300);
}
...
function genMain_table(){
	...
	code += '<input type="text" style="margin-left:10px;float:left;width:255px;" class="input_20_table" name="PC_devicename" onkeyup="device_filter(this);check_field();" placeholder="Please select the Device Group Name" autocorrect="off" autocapitalize="off" autocomplete="off" disabled>';
	...
	if(qos_bw_rulelist == ""){
		...
		for(k=0;k< qos_bw_rulelist_row.length;k++){
			...
			var apps_client_mac = qos_bw_rulelist_col[1].split("@")[1];
...
function applyRule(){
	...
	setRadioValue(document.form.qos_type, 2);
													...
													<script type="text/javascript">
														$('#radio_qos_enable').iphoneSwitch('<% nvram_get("qos_enable"); %>', 
															 function() {
																...
																if(getRadioValue(document.form.qos_type) != 2){
																	show_up_down(1);
																}
																else{
																	show_up_down(0);
										...
										<tr id="upload2_tr">
											...
											<td rowspan="2" style="width:250px;">
												<div>
													<ul style="padding:0 10px;margin:5px 0;">
														<li><#EzQoS_bandwidth_note1#></li>
														<li id="hint_zero2"><#EzQoS_bandwidth_note2#></li>
													</ul>
												</div>

	...
	var parseArrayToNvram = function(_dataArray) {
		...
		for(var i = 0; i < _dataArray.length; i += 1) {
			if(_dataArray[i].length != 0) {
				tmp_value += "<";
				for(var j = 0; j < _dataArray[i].length; j += 1) {
					tmp_value += _dataArray[i][j];
					if( (j + 1) != _dataArray[i].length)
						tmp_value += ">";
				}
			}
		}
...
function addRow_Group(upper, _this){
	...
	if(validForm(wan_idx)){
		...
		if(vts_rulelist_array_temp.length > 0) {
			...
			add_ruleList_array_temp.splice(1, 2);//filter Local IP and Local Port
			if(add_ruleList_array_temp[1] == "BOTH") { // BOTH is TCP and UDP
				for(var i = 0; i < vts_rulelist_array_temp.length; i += 1) {
					...
					currentRuleArrayTemp.splice(1, 2);//filter Local IP and Local Port
					if(add_ruleList_array_temp[0] == currentRuleArrayTemp[0] && currentRuleArrayTemp[1] != "OTHER") {
						alert("<#JS_duplicate#>");
				...
				for(var i = 0; i < vts_rulelist_array_temp.length; i += 1) {
					...
					if(currentRuleArrayTemp[1] == "BOTH") {
						if(add_ruleList_array_temp[0] == currentRuleArrayTemp[0]) {
							alert("<#JS_duplicate#>");
							document.getElementById("vts_port_x_" + wan_idx + "").focus();
							document.getElementById("vts_port_x_" + wan_idx + "").select();
							return;
...
function showvts_rulelist(_arrayData, _tableID) {
	...
	var width_array = [27, 15, 21, 10, 13];
			...
			if(eachValue.length != 0) {
				...
				for(var j = 0; j < eachValue.length; j += 1) {
					if(j != 0 && j != 1) {
						...
						code += handle_long_text(23, eachValue[j], width_array[j]);
					}
					else if(j == 1) {
						code += handle_long_text(13, eachValue[j], width_array[j]);
					}
				}
...
function gen_vts_ruleTable_Block(_tableID) {
	...
	html += '<tr>';
	html += '<th width="27%"><#BM_UserList1#></th>';
	html += '<th width="15%"><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,24);"><#FirewallConfig_LanWanSrcPort_itemname#></a></th>';
	html += '<th width="21%"><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,25);"><#IPConnection_VServerIP_itemname#></a></th>';
	html += '<th width="10%"><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,26);"><#IPConnection_VServerLPort_itemname#></a></th>';
	html += '<th width="13%"><#IPConnection_VServerProto_itemname#></th>';
	html += '<th width="14%"><#list_add_delete#></th>';
	html += '</tr>';
